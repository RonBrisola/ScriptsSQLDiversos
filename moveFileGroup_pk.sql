...
AND (dbo.sysindexes.indid = 1) 
--and (sysindexes.status & 64) = 0 and (not(sysindexes.status & 0x800)=0x800)  
and sysindexes.groupid<>1 
--and INDEXPROPERTY(dbo.sysobjects.id, dbo.sysindexes.name,'IsUnique') = 0
...
WHERE     (dbo.sysobjects.xtype = 'U') 
  ...
  and sysindexes.groupid<>1 AND SYSINDEXES.ROWCNT<=@ROWCNT
...
set  @list=left(@list,len(@list)-1)
	begin
      ...
      set @strsql='drop index ['+@tbname+'].['+@indname+']'
		...
		END
      */
		set @strsql='CREATE UNIQUE CLUSTERED INDEX  ['+@indname+'] ON [dbo].['+@tbname+']('+@list+') WITH  DROP_EXISTING, FILLFACTOR = 90  ON [PRIMARY]'
		print @strsql
		IF @DEBUG=0
		BEGIN
			exec sp_executesql @strsql
...
/*
SELECT INDEXPROPERTY(1989582126, 'PKTBCOMP001','IsUnique')
