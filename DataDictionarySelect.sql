...
DECLARE
   ...
   ,@ObjectName   AS nvarchar(128) = 'TBFATU010'

SELECT DatabaseName = @InstanceName +'.' +@DatabaseName
...
JOIN sys.types   AS ut ON c.user_type_id = ut.user_type_id
JOIN sys.types   AS st ON ut.system_type_id = st.user_type_id
WHERE o.type_desc NOT IN('INTERNAL_TABLE', 'SYSTEM_TABLE')
   ...
   and (@ObjectName IS NULL OR o.name LIKE @ObjectName)
ORDER BY
	CASE o.type_desc
