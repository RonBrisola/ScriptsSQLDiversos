sp_msforeachdb ' select ''?'', 
                (select count(name) from ?..sysobjects where name like ''bak%'') as NrTabelas, 
                (SELECT  CAST((SUM( DISTINCT au.Total_pages) * 8 ) / 1024.000 AS NUMERIC(18, 3)) 
                 ...
                 WHERE t.name like ''%bak%''
                   and t.Is_ms_shipped = 0 AND idx.Object_id > 255 ) AS [Tamanho] '
